...
def invoke_supervisor(user_input: str, thread_id: Optional[str] = None, context: Optional[list[dict]] = None) -> dict:
    ...
    if clean_input in ["hello", "hi", "hey", "sup", "yo"]:
        return {
            "response": "Hello! I am your Xeno CRM assistant. How can I help you build segments or coordinate your campaigns today?",
            "agent": "supervisor",
            "agents": [],
            "thread_id": thread_id,
        }
    ...
    try:
        ...
        kw = getattr(last, "additional_kwargs", {})
        if not isinstance(kw, dict):
            kw = {}
            
        return {
            "response": content,
            "agent": kw.get("agent", "supervisor"),
            "agents": kw.get("agents", []),
            "thread_id": thread_id,
        }
    except Exception as e:
        return {
            "response": "I ran into a processing error. Let me know what specific task you want to run next.",
            "agent": "supervisor",
            "agents": [],
            "thread_id": thread_id,
        }
